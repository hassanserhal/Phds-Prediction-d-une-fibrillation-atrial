...
Par exemple pour le patient 1 on a:
...
def read_record(path):
    ...
    for i in range (len(path)):
        ...
        record = wfdb.rdsamp(database+'/'+record_name)
...
def find_peak(array):
    ...
    for i in range(len(array)):
        x = np.array(array[i])
        peaks, _ = find_peaks(x, distance=100,threshold=0.01)
        np.diff(peaks)
        plt.plot(x)
        ...
        index.append(peaks)
    return index
Indice_peak_normal=find_peak(l_derv0_p_normal)
Indice_peak_sansAF=find_peak(l_derv0_p_sansAF)
Indice_peak_avantAF=find_peak(l_derv0_p_avantAF)
Indice_peak_avecAF=find_peak(l_derv0_p_avecAF)


#Concaténation des pics R.
signal=[]
concatenate_signal=[]
for i in range(25):
    indice_dernierpeaknormale=Indice_peak_normal[i][-1]
    signalnormalselectionne=l_derv0_p_normal[i][0:indice_dernierpeaknormale]
    indice_premierpeakRsansAF=Indice_peak_sansAF[i][0]
    indice_dernierpeakRsansAF=Indice_peak_sansAF[i][-1]
    signalsansselectionne=l_derv0_p_normal[i][indice_premierpeakRsansAF:indice_dernierpeakRsansAF]
    indice_premeiravantAF=Indice_peak_avantAF[i][0]
    signalavantAFselectionne=l_derv0_p_avantAF[i][indice_premeiravantAF::]
    signal=np.concatenate((signalnormalselectionne, signalsansselectionne,signalavantAFselectionne),axis=0)         
    concatenate_signal.append(signal)
...
x=0
try :
               os.mkdir("Base de données labélisée") 
except: 
               print("the folder exists") 
try :
               os.mkdir("Base de données labélisée/label0")               
except: 
               print("the folder exists") 
pathx="Base de données/label0"
patients=[]
for i in range(len(concatenate_signal)):
    while x >(38400 ):
     patients[i]=38400-signal[i]
