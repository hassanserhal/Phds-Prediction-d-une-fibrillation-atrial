...
for i in range(len(imfs_globale_derv0)):
       ...
       for j in range(min(len(list_imfs_normaux),len(list_imfs_anormaux))):
           ...
           l_variance.append(variance)
           standard_deviation=np.std(array_imf,axis=1)
           l_standard_deviation.append(standard_deviation)
           mean=np.mean(array_imf,axis=1)
           l_mean.append(mean)
           coeffiecient_variation= standard_deviation/mean
           l_coeffiecient_variation.append(coeffiecient_variation)
           significativit=significativity(coeffiecient_variation)
           if significativit== True :
               imfs_retenue.append([imf_normal,imf_anormal])
               if  j==0:
                   c0=c0+1
...
p0=(c0/(len (l_imfs_retenue_globale)))*100
p1=(c1/(len (l_imfs_retenue_globale)))*100
p2=(c2/(len (l_imfs_retenue_globale)))*100
p3=(c3/(len (l_imfs_retenue_globale)))*100
p4=(c4/(len (l_imfs_retenue_globale)))*100
p5=(c5/(len (l_imfs_retenue_globale)))*100
p6=(c6/(len (l_imfs_retenue_globale)))*100 
p7=(c7/(len (l_imfs_retenue_globale)))*100  
p8=(c8/(len (l_imfs_retenue_globale)))*100
p9=(c9/(len (l_imfs_retenue_globale)))*100 
p10=(c10/(len (l_imfs_retenue_globale)))*100 
p11=(c11/(len (l_imfs_retenue_globale)))*100  
p12=(c12/(len (l_imfs_retenue_globale)))*100 
print(p0,p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11,p12) 
print(c0,c1,c2,c3,c4,c5,c6,c7,c8,c9,c10,c11,c12)
print(c0+c1+c2+c3+c4+c5+c6+c7+c8+c9+c10+c11+c12)
print("Le pourcentage des IMFS de 0 à 12:{},{},\n{},{},\n{},{},\n{},{},\n{},{},\n{},{}".format(p0,p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11,p12))
l_variance1=[]; l_standard_deviation1=[];l_mean1=[];l_coeffiecient_variation1=[];l_imfs_retenue_globale1=[]
c0=c1=c2=c3=c4=c5=c6=c7=c8=c9=c10=c11=c12=0
p0=p1=p2=p3=p4=p5=p6=p7=p8=p9=p10=p11=p12=0

print("on travaille la dérivation 1")
for i in range(len(imfs_globale_derv1)):
       print(" on travaille le signal p"+str(i))
       ...
       for j in range(min(len(list_imfs_normaux1),len(list_imfs_anormaux1))):
           print(" on travaille l'imf "+str(j))
           imf_normal1=list_imfs_normaux1[j]
           imf_anormal1=list_imfs_anormaux1[j]
           array_imf1=np.array([imf_normal1,imf_anormal1])
           variance1=np.var(array_imf1,axis=1)
           l_variance1.append(variance1)
           standard_deviation1=np.std(array_imf1,axis=1)
           l_standard_deviation1.append(standard_deviation1)
           mean1=np.mean(array_imf1,axis=1)
           l_mean1.append(mean1)
           coeffiecient_variation1= standard_deviation1/mean1
           l_coeffiecient_variation1.append(coeffiecient_variation1)
           significativit=significativity(coeffiecient_variation1)
           if significativit== True :
               imfs_retenue1.append([imf_normal1,imf_anormal1])
               if  j==0:
                   c0=c0+1
       ...
       l_imfs_retenue_globale1.append(imfs_retenue1)
p0=(c0/(len (l_imfs_retenue_globale1)))*100
p1=(c1/(len (l_imfs_retenue_globale1)))*100
p2=(c2/(len (l_imfs_retenue_globale1)))*100
p3=(c3/(len (l_imfs_retenue_globale1)))*100
p4=(c4/(len (l_imfs_retenue_globale1)))*100
p5=(c5/(len (l_imfs_retenue_globale1)))*100
p6=(c6/(len (l_imfs_retenue_globale1)))*100 
p7=(c7/(len (l_imfs_retenue_globale1)))*100  
p8=(c8/(len (l_imfs_retenue_globale1)))*100
p9=(c9/(len (l_imfs_retenue_globale1)))*100 
p10=(c10/(len (l_imfs_retenue_globale1)))*100 
p11=(c11/(len (l_imfs_retenue_globale1)))*100  
p12=(c12/(len (l_imfs_retenue_globale1)))*100  
print(p0,p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11,p12) 
print(c0,c1,c2,c3,c4,c5,c6,c7,c8,c9,c10,c11,c12)
print(c0+c1+c2+c3+c4+c5+c6+c7+c8+c9+c10+c11+c12)
...
signal_entier=[]
for i in range(min(len(list_normale),len(list_anormale))):
    for j in range(len(list_normale)>38400,len(list_anormale)>38400):
        signal_entier=np.concatenate([list_normale_values[i].p_signals.T[0]],[list_normale_values[i].p_signals.T[1]],[list_anormale_values[i].p_signals.T[0]],[list_anormale_values[i].p_signals.T[1]])
        signal[j]=76800-signal[i]
        signal='csv'+str(j)
        path=path1+"signal/"
