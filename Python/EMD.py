...
#import LTAFDB data base
path='C:/Users/hp/Desktop/Présentation 5 -Travail avec EMD/MITBIH-LTAFD/LTAFDB'
#import data
headers=glob(path+'/*.hea')
atr=glob(path+'/*.atr')
qrs=glob(path+'/*.qrs')
signal=glob(path+'/*.dat')
#Open datfile
signals=[]
#for fname in signal :
#    signals.append(np.fromfile(fname,dtype=float))
s=np.fromfile(signal[9])  
#fig=plt.figure()
#plt.plot(s) 
ser=pd.Series(s)
df=pd.DataFrame(ser[:1000],columns=['s1'])
"""
...
if __name__ == "__main__":
 ...
 #path='C:/Users/hp/Desktop/Présentation 5 -Travail avec EMD/MITBIH-PAF/PAF'
