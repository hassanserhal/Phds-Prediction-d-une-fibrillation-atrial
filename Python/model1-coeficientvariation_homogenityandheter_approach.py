...
# EMD p01c-p02c
    ...
    list_mean=[]
    list_coeffiecient_variation=[]
    imfs_retenue_globale=[]
    ...
    for i in range(len(imfs_globale_derv1)) :
       ...
       for j in range(min(len(list_imfs_normaux1),len(list_imfs_anormaux1))):
           ...
           list_variance1.append(variance)
...
try :
            ...
            print("the folder exists")
for i in range(len(imfs_retenue_globale)):
     signal=imfs_retenue_globale[i] # imf p01c-p02c;p03c-p04c,...(normale;anormale)
     count=0
...
for i in range(len(imfs_retenue_globale)):
    if (imfs_retenue_globale[i]==imfs_retenue_globale[0]):
        c0=c0+1
        p0=(c0/len(imfs_retenue_globale))*100
    elif (imfs_retenue_globale[i]==imfs_retenue_globale[1]):
        c1=c1+1
        p1=(c1/len(imfs_retenue_globale))*100
    elif (imfs_retenue_globale[i]==imfs_retenue_globale[2]):
        c2=c2+1
        p2=(c2/len(imfs_retenue_globale))*100
    elif (imfs_retenue_globale[i]==imfs_retenue_globale[3]):
        c3=c3+1
        p3=(c3/len(imfs_retenue_globale))*100
    elif (imfs_retenue_globale[i]==imfs_retenue_globale[4]):
        c4=c4+1
        p4=(c4/len(imfs_retenue_globale))*100
    elif (imfs_retenue_globale[i]==imfs_retenue_globale[5]):
        c5=c5+1
        p5=(c5/len(imfs_retenue_globale))*100
    elif (imfs_retenue_globale[i]==imfs_retenue_globale[6]):
        c6=c6+1
        p6=(c6/len(imfs_retenue_globale))*100
    elif (imfs_retenue_globale[i]==imfs_retenue_globale[7]):
        c7=c7+1
        p7=(c7/len(imfs_retenue_globale))*100
    elif (imfs_retenue_globale[i]==imfs_retenue_globale[8]):
        c8=c8+1
        p8=(c8/len(imfs_retenue_globale))*100
    elif (imfs_retenue_globale[i]==imfs_retenue_globale[9]):
        c9=c9+1
        p9=(c9/len(imfs_retenue_globale))*100
    elif (imfs_retenue_globale[i]==imfs_retenue_globale[10]):
        c10=c10+1
        p10=(c10/len(imfs_retenue_globale))*100
    elif (imfs_retenue_globale[i]==imfs_retenue_globale[11]):
        c11=c11+1
        p11=(c11/len(imfs_retenue_globale))*100
    else: 
        c12=c12+1
        p12=(c12/len(imfs_retenue_globale))*100
print(p0,p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11,p12)
print(c0,c1,c2,c3,c4,c5,c6,c7,c8,c9,c10,c11,c12)
signalf=imfs_retenue_globale[i] # imf p01c-p02c;p03c-p04c,...(normale;anormale)
imft_normale=[]
imft_anormale=[]
...
#        signal1=signal_entier-600
#        signal2=signal1-600
#        signal3=signal2-600
#        signal4=signal3-600
#        signal5=signal4-600
#key,val=np.unique(imft_anormale,return_counts=True)   
#dict(zip(key,val))     
#deux entrées :
...
# un modèle pour chaque IMF


# Selection du meilleur modèle
#         p01 = records[i]
#         derv1 = p01.p_signals.T[i]
#         derv2 = p01.p_signals.T[1]
#         plt.plot(derv1)
